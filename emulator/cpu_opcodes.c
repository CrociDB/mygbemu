...
void cpu_init_table()
{
  ...
  optable[0xE9] =
      (opfunc_t){&cpu_op_e9, 0, 0,
                 0}; // ZEROES because JP already fill 4t when condition is met
...
void cpu_op_00(cpu_t* cpu, mmu_t* mmu) { debug_instruction(cpu, mmu, "NOP"); }
...
void cpu_op_e8(cpu_t* cpu, mmu_t* mmu)
{
  ...
  cpu_flag_set_halfcarry(cpu, ((cpu->reg.sp.word & 0x0FFF) + (value & 0x0FFF)) >
                                  0x0FFF);
