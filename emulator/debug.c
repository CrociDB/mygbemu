...
void debug_breakpoint_addr(cpu_t* cpu, uint16_t addr)
{
  ...
  debug_breakpoint_t* bp =
      (debug_breakpoint_t*)malloc(sizeof(debug_breakpoint_t));
...
void debug_loop(debugger_t* debugger)
{
  ...
  {
    log_debug("> 0x%04X\t%02X\t%s\n", pc_addr, (debugger->cpu->currop >> 8),
              debugger->current_disasm);
  }
  else
  {
    log_debug("> 0x%04X\t%04X\t%s\n", pc_addr, debugger->cpu->currop,
              debugger->current_disasm);
  }
  ...
  while (1)
  {
    ...
    else if (!strcmp(cmd, "r"))
    {
      printf("CPU REGISTERS\n"
             ...
             "\nCPU FLAGS\n"
             "\tZ: %d \tS: %d \tH: %d \tC: %d\n",
             debugger->cpu->reg.af.hi, debugger->cpu->reg.af.lo,
             debugger->cpu->reg.af.word, debugger->cpu->reg.bc.hi,
             debugger->cpu->reg.bc.lo, debugger->cpu->reg.bc.word,
             debugger->cpu->reg.de.hi, debugger->cpu->reg.de.lo,
             debugger->cpu->reg.de.word, debugger->cpu->reg.hl.hi,
             debugger->cpu->reg.hl.lo, debugger->cpu->reg.hl.word,
             debugger->cpu->reg.sp.word, pc_addr,
             cpu_flag(debugger->cpu, CPU_FLAG_ZERO_BIT),
             cpu_flag(debugger->cpu, CPU_FLAG_SUB_BIT),
             cpu_flag(debugger->cpu, CPU_FLAG_HC_BIT),
             cpu_flag(debugger->cpu, CPU_FLAG_CARRY_BIT));
    }
...
#ifdef WINDOWS
  strcpy_s(debugger->current_disasm, sizeof(debugger->current_disasm),
           "OPCODE MISSING");
#else
...
void debug_instruction(cpu_t* cpu, mmu_t* mmu, const char* disasm, ...)
{
  ...
  uint16_t pc_addr = (cpu->reg.pc.word - 1);
  bool isbreak =
      _debug_isbreak(debugger, pc_addr, buffer) || debugger->stopnext;
